...
public class FullDiffAdapter extends DiffAdapter {

    public FullDiffAdapter(DiffData oldData, DiffData newData, DiffWriter w ) throws IOException {
    ...
    public void format(Patch patch) throws IOException {
        ...
        int previousOldLast = -1; 
        int previousNewLast = -1; 
        for (Delta delta: patch.getDeltas()) {
            if ( previousOldLast + 1 < delta.getOriginal().getPosition() ) {
                List<String> oldLines = getOldLines().subList(previousOldLast + 1, delta.getOriginal().getPosition());
                List<String> newLines = getNewLines().subList(previousNewLast + 1, delta.getRevised().getPosition());
                w.newChange( 
                    ChangeType.COMMON,
                    previousOldLast + 1,
                ...
                w.append(
                    oldLines, previousOldLast + 1, 
                    newLines, previousNewLast + 1
                );
            }
            ...
            w.append(
                oldLines, delta.getOriginal().getPosition(), 
                newLines, delta.getRevised().getPosition()
            );
            previousOldLast = delta.getOriginal().last(); 
            previousNewLast = delta.getRevised().last(); 
        }
        if ( previousOldLast + 1 < getOldLines().size() ) {
            List<String> oldLines = getOldLines().subList(previousOldLast + 1, getOldLines().size());
            List<String> newLines = getNewLines().subList(previousNewLast + 1, getNewLines().size());
            w.newChange( 
                ChangeType.COMMON,
                previousOldLast + 1,
                getOldLines().size() - 1,
        ...
        if ( isOldMissingFinalLineSep() && isNewMissingFinalLineSep() ) {
            w.newChange(ChangeType.WARNING, getOldLines().size(), getOldLines().size(), getNewLines().size() , getNewLines().size());
            w.append( 
